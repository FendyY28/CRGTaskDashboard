  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[400px] bg-white rounded-3xl p-6 border-none shadow-2xl overflow-hidden">
        ...
        <div 
          style={{ backgroundColor: `${THEME.BSI_YELLOW}1A`, borderColor: `${THEME.BSI_YELLOW}33` }} 
          className="border p-4 rounded-2xl flex items-start gap-3 my-4"
        >
          <AlertCircle style={{ color: THEME.BSI_YELLOW }} className="h-5 w-5 mt-0.5" />
          <p style={{ color: THEME.BSI_YELLOW }} className="text-[11px] font-semibold leading-tight uppercase tracking-wider">
            {t('editProject.warnings.noteAutoSave')}
          </p>
        </div>

        <DialogFooter className="flex flex-row gap-3 pt-2">
          <Button variant="ghost" className="flex-1 rounded-2xl font-bold text-gray-400" onClick={() => onOpenChange(false)}>
            {t('editProject.buttons.cancel')}
          </Button>
          
          {/* 🔥 Menggunakan THEME.TOSCA untuk tombol utama. 
              Menggunakan hover:brightness-95 agar tidak perlu hardcode warna hover yang lebih gelap */}
          <Button 
            style={{ backgroundColor: THEME.TOSCA, boxShadow: `0 10px 15px -3px ${THEME.TOSCA}33` }} 
            className="flex-1 text-white rounded-2xl font-bold hover:brightness-95 transition-all" 
            onClick={onConfirm} 
            disabled={isLoading}
          >
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[420px] bg-white rounded-3xl p-6 border-none shadow-2xl overflow-hidden">
        ...
        <DialogFooter className="flex flex-row gap-3 pt-2">
          <Button variant="ghost" className="flex-1 rounded-2xl font-bold text-gray-400" onClick={() => onOpenChange(false)}>
            {t('editProject.buttons.cancel', 'Batal')}
          </Button>
