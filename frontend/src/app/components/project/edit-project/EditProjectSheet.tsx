...
import { NextCycleModal, RollbackModal } from "./EditProjectModals";
          ...
          <div className="bg-white border-b border-gray-100 p-6 flex items-start gap-4 sticky top-0 z-10">
            ...
            <div className="text-left flex-1">
              <DialogTitle className="text-xl font-bold text-gray-900 flex items-center justify-between">
                ...
                {project?.cycle > 1 && <Badge className="bg-[#36A39D]/10 text-[#36A39D] border-[#36A39D]/20 shadow-none">{t('editProject.cycleBadge', { cycle: project.cycle })}</Badge>}
              </DialogTitle>
          ...
          <form onSubmit={saveProjectChanges} className="p-6 space-y-6 text-left">
            ...
            <div className="grid grid-cols-2 gap-5">
              <div className="space-y-2">
                ...
                <Input 
                  ...
                  className={`bg-gray-50/50 border-gray-200 focus-visible:bg-white focus-visible:ring-0 focus-visible:ring-offset-0 focus-visible:border-[#36A39D] transition-all duration-300 ${errs.name ? "border-red-500" : ""}`} 
                />
              ...
              <div className="space-y-2">
                ...
                <Input 
                  ...
                  className="bg-gray-50/50 border-gray-200 focus-visible:bg-white focus-visible:ring-0 focus-visible:ring-offset-0 focus-visible:border-[#36A39D] transition-all duration-300"
                />
              </div>
            </div>
            ...
            <div className="p-5 bg-slate-50/80 rounded-xl border border-slate-100 space-y-5 relative overflow-hidden">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-2"><Activity className="h-5 w-5 text-[#36A39D]" /><h4 className="text-sm font-bold uppercase">{t('editProject.labels.phaseManagement')}</h4></div>
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Select value={form.currentPhase} onValueChange={handleSmartPhaseChange}>
                      <SelectTrigger className="bg-white h-9 border-gray-200 focus:ring-0 focus:ring-offset-0 focus:border-[#36A39D] transition-all duration-300">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent className="bg-white border-gray-100 shadow-xl rounded-xl">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="text-xs font-bold text-[#F9AD3C] bg-white border border-[#F9AD3C]/30 px-2 py-0.5 rounded">{form.overallProgress}%</span>
                    </div>
                    <Input 
                      type="range" 
                      ...
                      className="cursor-pointer accent-[#F9AD3C] h-2 bg-gray-200 rounded-lg appearance-none border-none focus:outline-none focus:ring-0 focus-visible:ring-0" 
                    />
                  </div>
                </div>
                  ...
                  {[{ l: t('editProject.labels.start'), v: form.phaseStartDate, k: "phaseStartDate", i: PlayCircle }, { l: t('editProject.labels.end'), v: form.phaseDeadline, k: "phaseDeadline", i: AlertCircle }].map((f, i) => (
                    <div key={i} className="space-y-1">
                      ...
                      <Input 
                        ...
                        className="h-8 text-xs bg-gray-50/50 border-gray-100 hover:border-gray-200 focus-visible:bg-white focus-visible:ring-0 focus-visible:ring-offset-0 focus-visible:border-[#36A39D] transition-all duration-300" 
                      />
                    </div>
                  ))}
              ...
              {[{ l: t('editProject.labels.projectStart'), v: form.projectStartDate, k: "projectStartDate" }, { l: t('editProject.labels.projectDeadline'), v: form.projectDeadline, k: "projectDeadline" }].map((f, i) => (
                <div key={i} className="space-y-2">
                  ...
                  <Input 
                    ...
                    value={f.v} 
                    onChange={e => handleChange(f.k, e.target.value)} 
                    className={`h-9 bg-gray-50/50 border-gray-200 focus-visible:bg-white focus-visible:ring-0 focus-visible:ring-offset-0 focus-visible:border-[#36A39D] transition-all duration-300 ${i===1 && errs.dates ? 'border-red-500' : ''}`} 
                  />
                </div>
              ))}
            ...
            <div className="space-y-2 bg-gray-50/50 p-3 rounded-xl border border-gray-100">
              ...
              <Select value={form.status} onValueChange={v => handleChange("status", v)}>
                <SelectTrigger className="bg-white h-10 capitalize border-gray-200 focus:ring-0 focus:ring-offset-0 focus:border-[#36A39D] transition-all duration-300">
                  <SelectValue/>
                </SelectTrigger>
                <SelectContent className="bg-white border-gray-100 shadow-xl rounded-xl">
            ...
            <div className="pt-2 flex flex-col sm:flex-row sm:justify-between items-center gap-4">
              <div className="w-full sm:w-auto">
                <Button type="button" onClick={() => setShowConfirm(true)} disabled={!isReadyForNextCycle || isLoading} className={`w-full sm:w-auto font-bold h-10 px-5 rounded-xl shadow-md gap-2 focus-visible:ring-0 focus-visible:ring-offset-0 transition-all duration-300 ${isReadyForNextCycle ? "bg-[#36A39D] hover:bg-[#2b8580] text-white hover:scale-105" : "bg-gray-100 text-gray-400 cursor-not-allowed"}`}><Rocket className="h-4 w-4" /> {isReadyForNextCycle ? t('editProject.buttons.startCycle', { cycle: (project?.cycle || 1) + 1 }) : isFormValidButUnsaved ? t('editProject.buttons.saveChangesFirst') : t('editProject.buttons.setLiveFirst')}</Button>
              </div>
              <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
                <Button type="button" variant="outline" onClick={() => onOpenChange(false)} className="rounded-xl border-gray-200 hover:bg-gray-50 focus-visible:ring-0 h-10">{t('editProject.buttons.cancel')}</Button>
                <Button type="submit" className="bg-[#36A39D] hover:bg-[#2b8580] text-white rounded-xl shadow-md font-bold px-8 focus-visible:ring-0 h-10 transition-all" disabled={isLoading}>{isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : t('editProject.buttons.saveChanges')}</Button>
              </div>
            </div>
          </form>
        </DialogContent>
